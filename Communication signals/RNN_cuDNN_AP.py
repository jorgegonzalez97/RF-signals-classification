...
from keras.layers import Input, Dense, Activation, ZeroPadding2D, BatchNormalization, Flatten, Conv2D, Conv1D, Convolution2D, Bidirectional, LSTM, GRU, CuDNNLSTM
...
X_train_f = np.zeros((n_train, 1024, 4), dtype=np.float32)
X_test_f = np.zeros((X_test.shape[0], 1024, 4), dtype=np.float32)
...
def RecComModel(input_shape):
    ...
    X = CuDNNLSTM(128, return_sequences= False, name = 'lstm1')(X)
...
output_path = '../Results/LSTM_APIQ_2/'
...
if Train:
    ...
    history = model.fit(X_train, Y_train, shuffle = True, batch_size = 512, callbacks = c, validation_data=(X_test, Y_test), epochs = 100)
...
acc={}
for snr in snrs:
    test_SNRs = list(map(lambda x: lbl_test[x][1], range(0,n_examples - n_train)))
